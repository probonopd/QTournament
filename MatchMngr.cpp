...
namespace QTournament {
  ...
  ERR MatchMngr::setSymbolicPlayerForMatch(const Match& fromMatch, const Match& toMatch, bool asWinner, int dstPlayerPosInMatch) const
  {
    ...
    if (toMatch.getState() != STAT_MA_INCOMPLETE) return ERR::MATCH_NOT_CONFIGURALE_ANYMORE;
  ...
  ERR MatchMngr::setPlayerToUnused(const Match& ma, int unusedPlayerPos, int winnerRank) const
  {
    ...
    if (ma.getState() != STAT_MA_INCOMPLETE) return ERR::MATCH_NOT_CONFIGURALE_ANYMORE;
  ...
  ERR MatchMngr::canAssignPlayerPairToMatch(const Match &ma, const PlayerPair &pp) const
  {
    ...
    if ((stat != STAT_MA_INCOMPLETE) && (stat != STAT_MA_FUZZY)) return ERR::MATCH_NOT_CONFIGURALE_ANYMORE;
  ...
  ERR MatchMngr::closeMatchGroup(const MatchGroup &grp)
  {
    ...
    if (grp.getState() != STAT_MG_CONFIG) return ERR::MATCH_GROUP_ALREADY_CLOSED;
  ...
  ERR MatchMngr::assignReferee(const Match& ma, const Player& p, REFEREE_ACTION refAction) const
  {
    ...
    static const std::string where{
      std::string{GENERIC_STATE_FIELD_NAME} + "=" + std::to_string(static_cast<int>(STAT_MA_READY)) +
      " OR " +
      std::string{GENERIC_STATE_FIELD_NAME} + "=" + std::to_string(static_cast<int>(STAT_MA_BUSY))
    };
  ...
  ERR MatchMngr::swapPlayer(const Match& ma, const PlayerPair& ppOld, const PlayerPair& ppNew) const
  {
    ...
    OBJ_STATE stat = ma.getState();
    if ((stat == STAT_MA_RUNNING) || (stat == STAT_MA_FINISHED)) return ERR::WRONG_STATE;
  ...
  void MatchMngr::updateAllMatchGroupStates(const Category& cat) const
  {
    ...
    WhereClause wc;
    wc.addCol(MG_CAT_REF, cat.getId());
    wc.addCol(GENERIC_STATE_FIELD_NAME, static_cast<int>(STAT_MG_SCHEDULED));
    ...
    wc.clear();
    wc.addCol(MG_CAT_REF, cat.getId());
    wc.addCol(GENERIC_STATE_FIELD_NAME, static_cast<int>(STAT_MG_FROZEN));
    for (TabRowIterator it{db, TAB_MATCH_GROUP, wc}; it.hasData(); ++it)
    {
      const MatchGroup mg{db, *it};
  ...
  {
    return (grp.getState() == STAT_MG_IDLE) ? ERR::OK : ERR::WRONG_STATE;
  }
  ...
  void MatchMngr::updateMatchStatus(const Match &ma) const
  {
    ...
    if (curState == STAT_MA_INCOMPLETE)
    {
      ...
      //
      // The definition of STAT_MA_FUZZY sais: "Player names are defined by symbolic values (e.g., winner of match XYZ); match number is assigned"
  ...
  ERR MatchMngr::assignMatchToCourt(const Match &ma, const Court &court) const
  {
    ...
    // become "BUSY"
    WhereClause wc;
    wc.addCol(GENERIC_STATE_FIELD_NAME, static_cast<int>(STAT_MA_READY));
  ...
  ERR MatchMngr::setMatchScoreAndFinalizeMatch(const Match &ma, const MatchScore &score, bool isWalkover) const
  {
    ...
    try
    {
      ...
      if (oldState == STAT_MA_RUNNING)
      {
        ...
        auto referee = ma.getAssignedReferee();
        if (referee && (referee->getState() == STAT_PL_REFEREE))
        {
      ...
      // become "READY"
      for (const Match& otherMatch : getObjectsByColumnValue<Match>(GENERIC_STATE_FIELD_NAME, static_cast<int>(STAT_MA_BUSY)))
      {
        updateMatchStatus(otherMatch);
      }
  ...
  ERR MatchMngr::undoMatchCall(const Match& ma) const
  {
    ...
    ma.row.update(cvc);
    CentralSignalEmitter::getInstance()->matchStatusChanged(ma.getId(), ma.getSeqNum(), STAT_MA_RUNNING, STAT_MA_READY);
  ...
  void MatchMngr::onPlayerStatusChanged(int playerId, int playerSeqNum, OBJ_STATE fromState, OBJ_STATE toState)
  {
    ...
    {
      for (const Match& ma : getObjectsByColumnValue<Match>(GENERIC_STATE_FIELD_NAME, static_cast<int>(STAT_MA_READY)))
      {
  ...
  void MatchMngr::resolveSymbolicNamesAfterFinishedMatch(const Match &ma) const
  {
    ...
    // FUZZY at least to WAITING, maybe even to READY or BUSY
    WhereClause wc;
    wc.addCol(GENERIC_STATE_FIELD_NAME, static_cast<int>(STAT_MA_FUZZY));
